                        ...
                        @if($employees->count())
                            @foreach($employees as $employee)
                                <tr>
                                    <td>{{$employee->id}}</td>
                                    <td>
                                    <img class="emp-img" src="{{$employee->picture}}">
                                    </td>
                                   <td> <a href=' {{url("attendence_create/$employee->id")}}'
                                    >{{$employee->first_name}} {{$employee->last_name}}</a></td>
                                    <td>{{$employee->empDepartment->dept_name}}</td>
                                    <td>{{$employee->empDesignation->emp_designation}}</td>
                                    <td>{{$employee->join_date}}</td>
                                    <td>
                                    <a href="{{route('attendence.show',$employee->id)}}" class="btn btn-small btn-floating waves=effect waves-light teal lighten-2"><i class="material-icons">list</i></a>
                                    </td>
                                </tr>
                            @endforeach
                            @if(isset($search))
